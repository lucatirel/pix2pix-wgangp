...
def main_inference(
    ...
    for index, filename in enumerate(os.listdir(noisy_imgs_path)):
            ...
            for i in range(patches.shape[0]):
                for j in range(patches.shape[1]):
                    ...
                    if generator.use_tanh:
                        ...
                        denoised_patch = np.where(denoised_patch > 0.5, 1, 0).astype(
                            np.uint8
                        )
        ...
        denoised_image = np.where(denoised_image >= 0.5, 1, 0).astype(np.uint8) * 255
...
checkpoint_path = R"C:\Users\Luca Tirel\Desktop\Denoising\model_best1.pth.tar"
testing_imgs_path = (
    R"C:\Users\Luca Tirel\Desktop\Denoising\datasets\denoise_testing_real"
)
